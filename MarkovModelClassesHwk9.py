...
import ParameterClassesHw9 as P
...
class PatientStateMonitor:
    ...
    def __init__(self, parameters):
        ...
        self._dead = False   # if the patient died
    ...
    def update(self, k, next_state):
        ...
        if self._currentState != P.HealthStats.Dead and next_state == P.HealthStats.Dead:
            self._dead = True
            self._dead = (k + 0.5) * self._delta_t  # corrected for the half-cycle effect
...
class CohortOutputs:
    def __init__(self, simulated_cohort):
        ...
        for patient in simulated_cohort.get_patients():
            ...
            # get the patient's time to death
                time_to_death = patient.get_time_to_death()
            if not (time_to_death is None):
                self.get_time_to_death.append(self.get_time_to_death)
        ...
        self._sumState_timeToAIDS = StatCls.SummaryStat('Time until AIDS', self._time_to_death)
